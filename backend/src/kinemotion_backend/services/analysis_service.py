...
class AnalysisService:
    ...
    async def analyze_video(
        ...
        logger.info("validating_video_file", filename=file.filename)
        ...
        try:
            ...
            logger.info("generating_storage_key", original_filename=file.filename)
            storage_key = await self.storage_service.generate_unique_key(
                file.filename or "video.mp4", user_id
            )
            logger.info("storage_key_generated", storage_key=storage_key)

            # Save uploaded file to temporary location
            logger.info("saving_uploaded_file", storage_key=storage_key)
