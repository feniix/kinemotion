    ...
    def process_frame(
        ...
        try:
            ...
            with self.timer.measure("optimized_cpu_detection"):
                ...
                if self.verbose:
                    print(f"DEBUG: det_outputs[0] shape: {det_outputs[0].shape}")
...
def create_optimized_cpu_tracker(
    mode: str = "lightweight",
    input_size: tuple[int, int] = (192, 256),
    intra_threads: int = 8,
    inter_threads: int = 1,
    ...
    """Factory function to create an optimized CPU RTMPoseTracker.

    Args:
        mode: Performance mode ('lightweight', 'balanced', 'performance')
        input_size: Pose input size as (height, width) - try (160, 160) or (128, 128) for speed
        intra_threads: Number of intra-op threads (default: 8 for Ryzen 7 7800X3D)
        inter_threads: Number of inter-op threads (default: 1)
        timer: Optional Timer for measuring operations

    Returns:
        Configured OptimizedCPUTracker instance

    Example:
        # Standard size (192x256)
        tracker = create_optimized_cpu_tracker()

        # Reduced size for speed (160x160)
        tracker = create_optimized_cpu_tracker(input_size=(160, 160))

        # Even smaller (128x128) - fastest but less accurate
        tracker = create_optimized_cpu_tracker(input_size=(128, 128))
    """
